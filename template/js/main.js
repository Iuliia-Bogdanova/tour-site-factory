...
// // === 2. Динамическое обновление CSS свойств === //
// function initDynamicCSSProps() {
//     function updateDynamicProperty(propertyName, minValue, maxValue, unit, minWidth, maxWidth) {
//         const windowWidth = Math.min(window.innerWidth, maxWidth);
//         const value = minValue + ((windowWidth - minWidth) * (maxValue - minValue)) / (maxWidth - minWidth);
//         document.documentElement.style.setProperty(`--${propertyName}`, `${value}${unit}`);
//     }

//     const updateMarginRight = () => updateDynamicProperty("margin-right", 6, 47, "rem", 360, 1023);

//     window.addEventListener("load", updateMarginRight);
//     window.addEventListener("resize", updateMarginRight);
// }

// // === 3. Слайдер вверху страницы (Swiper) === //
...
// // === 5. Анимация текста "Typed" в портфолио === //
...
//     const tabButtons = tabsContainer.querySelectorAll("button[data-category]");
//     const cardContainer = document.querySelector(".portfolio-tabs__cards");
//     const allCards = Array.from(cardContainer.querySelectorAll(".portfolio-card, .portfolio-card--tall"));

//     let activeTab = tabsContainer.querySelector(".btn-primary-tab");

//     const isActivePrimaryCurrent = () =>
//         activeTab.classList.contains("btn-primary-tab") && activeTab.classList.contains("btn-current");
...
//             allCards.forEach((card) => {
//                 const cardCategories = card.dataset.category.split(" ");
//                 const isMatch = category === "all" || cardCategories.includes(category);
//                 card.style.display = isMatch ? "" : "none";
//             });
...
//                 }
//             } else {
//                 cardContainer.classList.remove("disable-tall", "layout--narrow");
//                 cardContainer.style.columnCount = "";
//             }
//         });
//     });
...
// // === 7. Анимация роста круга и перекрытия текста === //
// function initGrowCircle() {
//     const growCircle = document.querySelector(".spacer-inner__grow-circle");
//     const section = document.querySelector(".section__spacer");
//     const stopScaleEl = document.querySelector(".stop-scale");
//     const fadeElements = document.querySelectorAll(".spacer-fade-text, .spacer-fade-svg");
...
//         const { top, height } = section.getBoundingClientRect();
//         const viewportHeight = window.innerHeight;
//         const rawProgress = (viewportHeight - top - triggerOffset) / (viewportHeight * 2);
//         const limitedProgress = Math.min(Math.max(rawProgress, 0), 1);
...
//         fadeElements.forEach((el) => {
//             const elRect = el.getBoundingClientRect();
//             const overlaps = elRect.right > circleLeft &&
//                              elRect.left < circleRight &&
//                              elRect.bottom > circleTop &&
//                              elRect.top < circleBottom;
//             el.classList.toggle("in-circle", overlaps);
...
//         // console.log({ scale, centerY, radius, stopTop, circleBottom, frozen: scaleFrozen });
//     }
